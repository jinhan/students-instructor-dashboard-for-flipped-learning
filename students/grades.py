...
def _progress_summary(student, request, course):
    ...
    for chapter_module in course_module.get_display_items():
        ...
        if chapter_module.hide_from_toc:
            continue
        #  전체 수업 전후에 실시하는 사전설문, 사후설문 메뉴 숨김
        if chapter_module.display_name_with_default in (u'사전설문',u'사후설문'):
            chapter_name = "hidden"
        else:
            chapter_name = chapter_module.display_name_with_default
	
        sections = [] # 각 챕터 안의 항목들 (예, 학습목표, 동영상강의, 퀴즈 ...)   
        for section_module in chapter_module.get_display_items():
    		...
    		if section_module.display_name_with_default in u'학습목표 및 문제확인':
			
                week_name = chapter_module.display_name_with_default
                if week_name == u'1주차 이러닝 콘텐츠 평가':
                        content = [u'이러닝 콘텐츠 평가 기준을 이해하고 예를 들어서 설명할 수 있다.', u'이러닝 콘텐츠를 평가 기준을 활용하여 교육용 웹사이트를 평가하고, 개선 방안을 제시할 수 있다.']
                elif week_name == u'2주차 프로토타입 개발':
                        content = [u'프로토타입의 개념과 특징을 설명할 수 있다.', u'프로토타입의 개발 절차를 설명할 수 있다.', u'이러닝 모듈의 페이퍼 프로토타입을 개발할 수 있다.']
                else:
                        content = [u'사용성 평가의 개념 및 필요성을 설명할 수 있다.', u'사용성 평가의 절차에 따라 평가를 설계하고 실행할 수 있다.', u'사용성 평가를 통해 도출한 결과를 분석하여 보고서로 작성할 수 있다.']
    		...
    		elif section_module.display_name_with_default in u'동영상 강의 및 학습자료':
                sections.append({
                'display_name': section_module.display_name_with_default,
                'url_name': section_module.url_name,
                })
    			...
    			quizs = {
    			    'display_name': u'생각해보기',
    			    'url_name': section_module.url_name,
    			}
    			sections.append(quizs)

    		elif section_module.display_name_with_default in u'개인해결안':
    			sections.append({
    				'display_name': section_module.display_name_with_default,
    				'url_name': section_module.url_name,
    			})

    		elif section_module.display_name_with_default in u'성찰':
            ...
            # 기존 progress 페이지에서 보여지는 퀴즈 점수 데이터
    		else:
