...
# jinhan
import courseware.dashboard_data as dashboard
import courseware.logs2mongo as logs2mongo
import pymongo
# jinhan end
...
def _progress(request, course_key, student_id):
    ...
    # jinhan
    logs2mongo.logs2mongo() # 대시보드 출력하기 전에 로그에서 mongodb로 아직 안쓰인 것들 찾아서 쓰기
    # jinhan end
    ...
    # courseware_summary, dashboard_overview_summary = grades.progress_summary(student, request, course) 
    courseware_summary = grades.progress_summary(student, request, course)
    
    # jinhan
    # 대시보드에 출력할 학습자들의 활동 데이터 가져오기
    dashboard_overview_data = dashboard.get_dashboard()
    # jinhan end
    ...
    context = {
        ...
        'dashboard_overview_data': dashboard_overview_data, # 대시보드 데이터를 template에서 쓰도록 context에 추가
    }
    ...
    with grades.manual_transaction():
        response = render_to_response('courseware/progress.html', context) # 대시보드 데이터가 progress.html 템플릿으로 전달
