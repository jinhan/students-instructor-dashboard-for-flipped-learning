...
# jinhan
import courseware.dashboard_data as dashboard_data
...
def instructor_dashboard_2(request, course_id):
    ...
    sections.append(_section_dashboard(course,access))
...
## jinhan
# dashboard section 추가
# instructor_dashboard_2.html에서 section_key를 읽어서 (section_key).html을 render
def _section_dashboard(course, access):
    """ Provide data for the corresponding dashboard section """
    logs2mongo.logs2mongo() # 교수자 대시보드 그리기 전에 아직 mongo에 추가되지 않은 log 추가
    course_key = course.id

    section_data = {
        'section_key': 'dashboard',
        'section_display_name': _('Dashboard'),
        'access': access,
        ...
        'instructor_dashboard_data': dashboard_data.get_dashboard(), # instructor 대시보드에 필요한 데이터를 context에 추가
    }
